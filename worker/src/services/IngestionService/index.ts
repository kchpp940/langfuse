...
export class IngestionService {
  ...
  public async mergeAndWrite(
    ...
    let outcomes: IngestionEventOutcome[] = [];

    try {
      switch (eventType) {
        case "trace":
          outcomes = await this.processTraceEventList({
            projectId,
          ...
          break;
        case "observation":
          outcomes = await this.processObservationEventList({
            projectId,
            entityId: eventBodyId,
            createdAtTimestamp,
          ...
          break;
        case "score":
          outcomes = await this.processScoreEventList({
            projectId,
            entityId: eventBodyId,
            createdAtTimestamp,
        ...
        case "dataset_run_item":
          outcomes = await this.processDatasetRunItemEventList({
            projectId,
            entityId: eventBodyId,
            createdAtTimestamp,
        ...
        default:
          outcomes = events.map((event) =>
            this.createOutcome(
              ...
              eventBodyId,
              projectId,
              IngestionEventOutcomeStatus.SKIPPED,
              IngestionEventOutcomeStage.VALIDATION,
              new Error(`Unknown event type: ${eventType}`),
            ),
          );
      }
    } catch (unexpectedError) {
      logger.error(
        `Unexpected error in mergeAndWrite for project ${projectId}, eventBody ${eventBodyId}, type ${eventType}`,
        unexpectedError,
      );

      const coveredEventIds = new Set(outcomes.map((o) => o.eventId));
      const fallbackOutcomes = events
        ...
        .map((event) =>
          this.createOutcome(
            ...
            eventBodyId,
            projectId,
            IngestionEventOutcomeStatus.FAILED,
    ...
    const finalEventIds = new Set(outcomes.map((o) => o.eventId));
    const missingEventIds = events
      .map((e) => e.id)
      .filter((id) => !finalEventIds.has(id));
    if (missingEventIds.length > 0) {
      logger.warn(
        `Outcome invariant violated: ${missingEventIds.length} events missing outcomes for project ${projectId}, eventBody ${eventBodyId}`,
        { missingEventIds },
      );
      for (const event of events.filter((e) =>
        missingEventIds.includes(e.id),
      )) {
        outcomes.push(
          this.createOutcome(
            event,
            ...
            IngestionEventOutcomeStatus.SKIPPED,
            IngestionEventOutcomeStage.VALIDATION,
            new Error("Outcome not generated by processor"),
          ),
        );
      }
    }
  ...
  private async processDatasetRunItemEventList(params: {
    ...
    const outcomes: IngestionEventOutcome[] = [];

    if (datasetRunItemEventList.length === 0) return outcomes;
        ...
        try {
          ...
          if (!runData || !itemData) {
            return {
              event,
              records: [],
              error: new Error("Dataset run or item not found"),
            };
          }
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      const event = datasetRunItemEventList[i];
      ...
      const { records, error } = result.value;
      ...
      if (error || records.length === 0) {
        outcomes.push(
          this.createOutcome(
            ...
            IngestionEventOutcomeStatus.FAILED,
            IngestionEventOutcomeStage.VALIDATION,
            error ?? new Error("No records generated"),
          ),
        );
        continue;
      }
      ...
      try {
        ...
        outcomes.push(
          this.createOutcome(
            ...
            entityId,
            projectId,
            IngestionEventOutcomeStatus.SUCCESS,
            IngestionEventOutcomeStage.WRITE,
          ),
        );
      } catch (writeError) {
        outcomes.push(
  ...
  private async processScoreEventList(params: {
    ...
    if (scoreEventList.length === 0) return outcomes;
    ...
    const validationResults = await Promise.all(
      timeSortedEvents.map(async (scoreEvent) => {
        try {
          ...
          return { event: scoreEvent, record: null, error };
        }
      }),
    );

    const successfulRecords: ScoreRecordInsertType[] = [];
    ...
    try {
      ...
      if (successfulRecords.length === 0 && !clickhouseScoreRecord) {
        return outcomes;
      }
      ...
      logger.error(
        `Failed to merge/write score for project ${projectId}, score ${entityId} at stage ${mergeStage}`,
        error,
      );
      for (const result of validationResults) {
        if (result.record) {
  ...
  private async processTraceEventList(params: {
    ...
    if (traceEventList.length === 0) return outcomes;
    ...
    try {
      ...
      const finalIO = {
        ...
        output: this.stringify(
          reversedRawRecords.find((record) => record?.body?.output)?.body
            ?.output,
        ),
      };
      ...
      currentStage = IngestionEventOutcomeStage.MERGE;
      ...
      currentStage = IngestionEventOutcomeStage.WRITE;
      ...
      for (const event of traceEventList) {
        outcomes.push(
          this.createOutcome(
            event,
            entityId,
      ...
      logger.error(
        `Failed to process trace events for project ${projectId}, trace ${entityId} at stage ${currentStage}`,
        error,
      );
      for (const event of traceEventList) {
        outcomes.push(
  ...
  private async processObservationEventList(params: {
    ...
    try {
      ...
      const rawOutput =
        reversedRawRecords.find((record) => record?.body?.output)?.body
          ?.output ?? clickhouseObservationRecord?.output;
      ...
      for (const event of observationEventList) {
        outcomes.push(
          this.createOutcome(
            event as unknown as IngestionEventType,
            entityId,
            projectId,
            IngestionEventOutcomeStatus.SUCCESS,
            IngestionEventOutcomeStage.WRITE,
      ...
      logger.error(
        `Failed to process observation events for project ${projectId}, observation ${entityId} at stage ${currentStage}`,
        error,
      );
      for (const event of observationEventList) {
        outcomes.push(
